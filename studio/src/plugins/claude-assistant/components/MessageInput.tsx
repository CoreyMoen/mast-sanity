  ...
  function MessageInput(
    ...
    const handleContainerClick = useCallback((e: React.MouseEvent) => {
      if (
        ...
        (e.target as HTMLElement).closest('[data-input-area]')
      ) {
        textAreaRef.current?.focus()
      }
    }, [])
    ...
    return (
      ...
      >
        ...
        <div
          data-input-area
          style={{padding: isCompact ? '10px 12px 6px 12px' : '16px 16px 8px 16px'}}
        >
          <textarea
            ref={textAreaRef}
            value={value}
        ...
        >
          ...
          <Flex align="center" gap={2}>
            <Tooltip
              content={
                <Box padding={2}>
                  <Text size={1}>{canSend ? 'Send message' : 'Type a message'}</Text>
                </Box>
              }
              placement="top"
              portal
