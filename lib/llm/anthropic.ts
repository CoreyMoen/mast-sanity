...
export class AnthropicProvider implements LLMProvider {
  ...
  constructor(apiKey: string, model: string = DEFAULT_MODEL) {
    if (!apiKey) {
      throw new Error(
        "Anthropic API key is required. Provide a valid API key from the Anthropic console.",
      );
    }
    this.apiKey = apiKey;
    this.model = model;
  ...
  private async callAnthropic(
    ...
    try {
      ...
      throw new Error(
        `Anthropic API request failed: ${error instanceof Error ? error.message : "Network error"}`,
      );
    }
    ...
    if (!text) {
      throw new Error(
        "Anthropic API returned an empty response. The model may have refused the request.",
      );
    }
